...
const phases = [
  ["01", "Explore", "Find the right problem to solve", "Turn lived experience and market observations into a focused business opportunity worth testing.", "Idea validation Â· Founder readiness Â· Market discovery"],
  ["02", "Plan", "Build a practical path forward", "Clarify the customer, offer, business model, milestones, and decisions that come before launch.", "Business model Â· Mentor support Â· Planning tools"],
  ["03", "Launch", "Move from plan to first customer", "Choose the essentials, formalize the business, and bring the first version of the offer to market.", "Business setup Â· First offer Â· Go-to-market"],
  ["04", "Operate", "Create a business that runs clearly", "Connect front-office activity to the systems, people, technology, and routines behind delivery.", "Core operations Â· Technology stack Â· Financial rhythm"],
  ["05", "Grow", "Build repeatable momentum", "Strengthen acquisition, delivery, partnerships, and capacity without losing control of the operation.", "Growth channels Â· Team capacity Â· Capital readiness"],
  ["06", "Optimize", "Make the whole system work better", "Review results, remove friction, and direct time and resources toward what creates the most value.", "Performance review Â· Process improvement Â· Next-stage strategy"],
];
  ...
  return <main>
    <header className="header">
      ...
      <nav className={menu ? "open" : ""}>
        <a href="#about">About</a><a href="#how">How it works</a><a href="#resources">Resources</a><a className="navCta" href="#newsletter">Join the pilot â†—</a>
      </nav>
    </header>

    <section className="hero" id="top">
      <div className="heroGrid">
        <div><p className="eyebrow"><span>Veteran-focused pilot</span> Â· Opening 2026</p>
          <h1>Stop searching.<br/><em>Start navigating.</em></h1>
          <p className="lead">BOSS Libraries turns a fragmented business-support ecosystem into a clear pathâ€”so owner-operators can find the right resource, at the right phase, for the right next move.</p>
          <div className="actions"><a className="button" href="#newsletter">Get the sample playbook â†’</a><a className="textLink" href="#how">See how it works â†“</a></div>
        </div>
      ...
      <div className="proof"><p>Built for owner-operators who need a pathâ€”not more tabs.</p><div><b>6</b> business phases</div><div><b>1</b> playbook</div><div><b>0</b> pay-to-play rankings</div></div>
    </section>
    ...
    <section className="section twoCol" id="about">
      ...
      <div className="copy"><p>Entrepreneurs rarely struggle because nobody wants to help. They struggle because programs, people, tools, and opportunities are spread across an ecosystem with no shared map.</p><p>BOSS Libraries is the connective tissue: a trusted education and navigation layer that helps people understand what exists, when it matters, and how the pieces work together.</p><blockquote>â€œStrong ecosystems arenâ€™t built by adding more resources. Theyâ€™re built by connecting the right ones.â€</blockquote></div>
    </section>
    ...
    <section className="section" id="how">
      ...
      <div className="phaseBox">
        ...
        <div className="phase"><p className="kicker">Phase {phases[active][0]}</p><h3>{phases[active][2]}</h3><p>{phases[active][3]}</p><div className="chips">{phases[active][4].split(" Â· ").map(x=><span key={x}>{x}</span>)}</div><a href="#newsletter">Preview this phase in the playbook â†—</a></div>
      </div>
    </section>

    <section className="ecosystem"><div className="section">
      <Heading light kicker="An ecosystem that works together" title={<>Force. Capability.<br/><em>Navigation.</em></>} text="BOSS is designed to strengthen the organizations already doing the workâ€”not replace them." />
    ...
    <section className="section" id="resources">
      ...
      <div className="partnerGrid">{partners.map(p=><a href={p[3]} target="_blank" rel="noreferrer" key={p[1]}><span>{p[0]}</span><h3>{p[1]}</h3><p>{p[2]}</p><b>Explore resource â†—</b></a>)}</div>
    </section>

    <section className="section founder">
      <div className="portrait"><div/><strong>DD</strong><p>Founder &amp;<br/>Ecosystem Architect</p></div>
      <div><p className="kicker">Meet the founder</p><h2>Doug is mapping the system between the systems.</h2><p>BOSS Libraries grew from a simple field observation: entrepreneurs are surrounded by capable organizations, but they still lose time trying to determine where to start and what comes next.</p><p>Dougâ€™s work focuses on making the small-business ecosystem more visible, connected, and usefulâ€”beginning with veteran owner-operators in the Houston region.</p><a className="textLink dark" href="#connect">Connect with Doug â†’</a></div>
    </section>
    ...
    <section className="newsletter" id="newsletter"><div><p className="kicker">Join the BOSS briefing</p><h2>Know your phase.<br/><em>Find your next move.</em></h2><p>Get pilot updates, ecosystem insights, and the sample Navigational Playbook.</p>
      {joined?<div className="success" role="status">âœ“ <span><b>Youâ€™re on the early-access list.</b><br/>Watch your inbox for the first BOSS briefing.</span></div>:<form onSubmit={join}><input type="email" required placeholder="Email address" aria-label="Email address"/><button>Join early access â†’</button></form>}
      <small>By joining, you agree to receive BOSS Libraries updates. Unsubscribe anytime.</small></div>
    </section>
    <footer><div><strong>B</strong><span><b>BOSS Librariesâ„¢</b><small>Business navigation for owner-operators.</small></span></div><nav><a href="#about">About</a><a href="#how">How it works</a><a href="#resources">Resources</a><a href="#newsletter">Newsletter</a></nav><p>Â© 2026 BOSS Libraries. Pilot experience.</p></footer>
  </main>;
}
...
function Path({n,who,title,text}:{n:string,who:string,title:string,text:string}) { return <article><span>{n}</span><p>{who}</p><h3>{title}</h3><small>{text}</small><a href="#newsletter">Start here â†’</a></article> }
